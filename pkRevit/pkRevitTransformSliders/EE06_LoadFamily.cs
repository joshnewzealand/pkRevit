...
namespace RevitTransformSliders
{
    ...
    public class EE06_PlaceFamily : IExternalEventHandler  //this is the last when one making a checklist change, EE4 must be just for when an element is new
    {
        ...
        public void Execute(UIApplication uiapp)
        {
            ...
            {
                _952_PRLoogleClassLibrary2.DatabaseMethods.writeDebug("EE06_PlaceFamily" + Environment.NewLine + ex.Message + Environment.NewLine + ex.InnerException, true);
            }
            finally
            {
            }
        ...
        public void Execute(UIApplication uiapp)
        {
            try
            {
                ...
                using (Transaction tx = new Transaction(doc))
                {
                    ...
                    if (myWindow1.myThisApplication.messageConst.Split('|')[0] == "Release")  //constructs a path for release directory (in program files)
                    {
                        myString_TempPath = myWindow1.myThisApplication.messageConst.Split('|')[1] + "//Families";
                    }
                    if (myWindow1.myThisApplication.messageConst.Split('|')[0] == "Dev") //constructs a path for development directory
                    {
                        myString_TempPath = myWindow1.myThisApplication.messageConst.Split('|')[1] + "//Families";
                    }
